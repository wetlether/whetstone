// Whetstone box-side grading — runs INSIDE the user's Wetlether box, on the box's own Claude (the user's own API key).
// The box passes its existing model-call as `callClaude` (so this stays decoupled from box internals,
// uses the user's own API key = private; the user pays their provider per use). Returns per-axis scores; the score.html
